...
export function CodeEditor({
  ...
  return (
    <div className="flex flex-col h-full bg-app-panel backdrop-blur-xl">
      {/* Open tabs & Toolbar */}
      <div className="flex items-center justify-between px-1 py-1 border-b border-border-subtle bg-app-bg/50 overflow-x-auto">
        ...
        {/* Quick Run button in editor tab header */}
        <div className="flex items-center gap-1.5 px-2 flex-shrink-0">
          <button
            onClick={onRunCode}
            disabled={isRunningCode}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold font-mono transition-all bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 hover:bg-emerald-500/30 hover:scale-105 flex-shrink-0 shadow-sm"
            title="Run Code (Ctrl+Enter)"
          >
            <Play size={11} className={isRunningCode ? "animate-spin" : "fill-emerald-400"} />
      ...
      {/* Breadcrumb */}
      {current && (
        <div className="flex items-center justify-between px-3 py-1 border-b border-border-subtle text-[11px] text-fg-muted overflow-x-auto">
          ...
          <div className="text-[10px] text-fg-faint font-mono">
            Press <kbd className="px-1 py-0.5 rounded bg-black/40 border border-white/10 text-cyan-300">Ctrl+Enter</kbd> to Run
          </div>
        </div>
      )}

      {/* Editor area */}
      <div className="flex-1 flex overflow-hidden font-mono text-xs relative min-h-[140px]">
        {/* Line numbers */}
        <div
          ref={lineNumbersRef}
      ...
      {/* Bottom info bar */}
      {current && (
        <div className="flex items-center justify-between px-3 py-1 border-t border-border-subtle text-[10px] text-fg-muted bg-app-bg/30 flex-shrink-0">
      ...
      {/* Integrated IDE Execution Console Drawer */}
      <ExecutionConsole
        logs={executionLogs}
